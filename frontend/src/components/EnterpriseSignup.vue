...
<script>
  export default {
    data () {
      return {
        messages: [{'content': '你好'}, {'content': '我好'}],
        show: false,
        warn: false,
        warnMes: '',
        value: '',
    ...
    methods: {
      ...
      fetchData () {
        fetch('/api/test/', {
          ...
          body: JSON.stringify({'type': 'inner'})
        })
        .then((res) => res.json())
        .then((res) => {
      ...
      send () {
        ...
        this.messages.push({'content': this.value})
        this.value = ''
        fetch('/api/talk/', {
          method: 'post',
          credentials: 'same-origin',
          ...
          body: JSON.stringify({'mes': this.value})
        })
        .then((res) => res.json())
        .then((res) => {
          this.messages.push(res['res'])
          var li = document.getElementById('convo')
