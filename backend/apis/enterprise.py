    ...
    **返回值**: 包含成功/失败信息的JsonResponse\n
    ...
    try:
        ...
        models.Enterprise.objects.create(
            ...
            name = info_dict['name'], robot_icon = info_dict['ri'], robot_name = info_dict['rn'],
            robot_state = info_dict['rs'], salt = info_dict['salt'])
        return JsonResponse({'flag': const_table.const.SUCCESS, 'message': ''})
    except Exception:
...
def set_customer_message(email, EID):
    ...
    **返回值**: 包含成功/失败信息和客服信息的JsonResponse\n
    ...
    models.Customer.objects.create(
        CID = CID, EID = EID, email = email, password = password,
    ...
    **返回值**: 包含成功/失败信息的JsonResponse\n
    ...
    try:
        ...
        if len(enterprise) > 0:
            ...
            models.Customer.objects.filter(email = email).update(
                password = password_salt['password'], salt = password_salt['salt'])
...
def enterprise_send_user_info(request):
    ...
    * **request** - 前端发送的请求，包含用户的id 和信息\n
    **返回值**:包含成功/失败消息的JsonResponse
    """
    information = json.loads(request.body.decode('utf8'))
    if 'uid' in information and 'info' in information:
        UID = information['uid']
